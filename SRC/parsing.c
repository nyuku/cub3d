...
void	*init_floor_ceiling(int *rgb, t_color *up_or_down, char *letter)
{
	if (letter == 'C')
	{
...
//recupere la str->int avec les check.
...
void	*set_floor_ceiling(char *str, t_color *up_or_down, char *letter)
{
	int	*rgb;
	
	rgb = set_color(str); // tableau de int avec le bon chiffre
	init_floor_ceiling(rgb,letter);
...
int	set_texture(char *str, t_map *map)
{
	...
	{
		// on recupere la str avec l'indice F ou C
		set_color(str, map);
		printf("its a F and C\n");
	}
